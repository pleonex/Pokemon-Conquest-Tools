...
namespace AmbitionText
{
	public class PNAWriter
	{
		...
		public PNAWriter(string text, bool useTable)
		{
			Create_Table();
		...
		byte[] Encode_Table(char c)
		{
            ...
            else
            {
                ...
                Console.WriteLine("Error encoding char {0} ({1})", c, BitConverter.ToString(Encoding.UTF8.GetBytes(c.ToString())));
